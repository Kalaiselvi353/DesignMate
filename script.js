...
setActiveSlide(0);
resizeCanvas();
drawGemDust();/*const deck = document.querySelector("#deck");
...
function setActiveSlide(index) {
    ...
    if (cue) {
        cue.style.opacity = index === slides.length - 1 ? "0" : "1";
        cue.style.pointerEvents = index === slides.length - 1 ? "none" : "auto";
    }
}
...
dots.forEach((dot, dotIndex) => {
    dot.addEventListener("click", (event) => {
        ...
        deck.scrollTo({ top: slides[dotIndex].offsetTop, behavior: "smooth" });
    });
});
...
window.addEventListener("keydown", (event) => {
    ...
    if (previousKeys.includes(event.key)) {
        ...
        deck.scrollTo({ top: slides[previousIndex].offsetTop, behavior: "smooth" });
    }
});
